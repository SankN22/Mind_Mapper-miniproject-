...
class Sheet:
		...
		if shiftType == 0:
			...
			for t in self.thoughts:
				...
				if not t.groupShifted:
					...
					direction = utils.getDir(node.pixLoc, t.pixLoc)

					cs = utils.cosSim(utils.normalize(delta), direction)
	...
	def nodeDist(self, tA, tB):
		return utils.dist(tA.pixLoc, tB.pixLoc)
