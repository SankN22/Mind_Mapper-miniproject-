...
def addLeave(event=[], stage=0):
    ...
    if stage <= total_stages:
        ...
        # toHex(textColour))
        tk_text.configure(fg=toHex(fontColour), bg=toHex(bgColour))
...
def addFile(event=[]):
    ...
    fontColour = toHex(
        shadeN([(1, 1, 1), cs.darkText], [0, 1], cs.fontOpacity))

    tk_text.configure(font=(settings.MAIN_FONT, fontSize,
                      "normal"), fg=fontColour, bg="white")
...
def sheetClick(filename, event=[]):
    ...
    if filename == '+':
        filename = tk_text.get('1.0', 'end').strip().replace(
            ' ', '_').split('.')[0]
...
def labelEnter(sheet, event=[]):
    ...
    sheet.configure(bg="white", fg=toHex(
        shadeN([(1, 1, 1), cs.darkText], [0, 1], cs.fontOpacity)))
...
def pulse(sheet, stage=0):
    ...
    try:
        ...
        lightFontColour = shadeN([cs.background, cs.lightText], [
                                 0, 1], cs.fontOpacity)
        darkFontColour = shadeN([(1, 1, 1), cs.darkText], [
                                0, 1], cs.fontOpacity)
...
def init_pages():
    ...
    for s in sheets_info:
        s_box = tk.Label(tk_root, text=s['name'], font=settings.FONT, bg=toHex(
            cs.background), fg=font_colour, cursor='hand1', anchor=tk.CENTER)
        s_box.bind('<Button-1>', lambda event,
                   filename=s['filename']: sheetClick(filename, event))
        s_box.bind('<Button-3>', lambda event,
                   sheet=s: sheetRightClick(sheet, event))

        s_box.bind('<Enter>', lambda event,
                   sheet=s_box: labelEnter(sheet, event))
        s_box.bind('<Leave>', lambda event,
                   sheet=s_box: labelLeave(sheet, event))

        tk_sheets.append(s_box)

    s_box_plus = tk.Label(tk_root, text='+', font=settings.FONT, bg=toHex(
        cs.background), fg=font_colour, cursor='hand1', anchor=tk.CENTER)
    s_box_plus.bind('<Button-1>', addFile)
    s_box_plus.bind('<Enter>', lambda event,
                    sheet=s_box_plus: labelEnter(sheet, event))
    s_box_plus.bind('<Leave>', lambda event,
                    sheet=s_box_plus: labelLeave(sheet, event))
...
if __name__ == "__main__":
    ...
    tk_text.bind('<Return>', lambda event,
                 filename='+': sheetClick(filename, event))
